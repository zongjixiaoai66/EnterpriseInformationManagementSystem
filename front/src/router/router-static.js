import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'

     import users from '@/views/modules/users/list'
    import bumenjihua from '@/views/modules/bumenjihua/list'
    import daka from '@/views/modules/daka/list'
    import dictionary from '@/views/modules/dictionary/list'
    import gerenjihua from '@/views/modules/gerenjihua/list'
    import gonggao from '@/views/modules/gonggao/list'
    import huiyi from '@/views/modules/huiyi/list'
    import qingjia from '@/views/modules/qingjia/list'
    import qiyejihua from '@/views/modules/qiyejihua/list'
    import tongxunlu from '@/views/modules/tongxunlu/list'
    import waichudengji from '@/views/modules/waichudengji/list'
    import yuangong from '@/views/modules/yuangong/list'
    import dictionaryBumen from '@/views/modules/dictionaryBumen/list'
    import dictionaryBumenjihua from '@/views/modules/dictionaryBumenjihua/list'
    import dictionaryDaka from '@/views/modules/dictionaryDaka/list'
    import dictionaryDakaZhuangtai from '@/views/modules/dictionaryDakaZhuangtai/list'
    import dictionaryGerenjihua from '@/views/modules/dictionaryGerenjihua/list'
    import dictionaryGonggao from '@/views/modules/dictionaryGonggao/list'
    import dictionaryHuiyi from '@/views/modules/dictionaryHuiyi/list'
    import dictionaryQingjia from '@/views/modules/dictionaryQingjia/list'
    import dictionaryQingjiaYesno from '@/views/modules/dictionaryQingjiaYesno/list'
    import dictionaryQiyejihua from '@/views/modules/dictionaryQiyejihua/list'
    import dictionarySex from '@/views/modules/dictionarySex/list'
    import dictionaryWaichudengji from '@/views/modules/dictionaryWaichudengji/list'
    import dictionaryZhiwei from '@/views/modules/dictionaryZhiwei/list'





//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    } ,{
        path: '/users',
        name: '管理信息',
        component: users
      }
    ,{
        path: '/dictionaryBumen',
        name: '部门',
        component: dictionaryBumen
    }
    ,{
        path: '/dictionaryBumenjihua',
        name: '部门计划类型',
        component: dictionaryBumenjihua
    }
    ,{
        path: '/dictionaryDaka',
        name: '打卡类型',
        component: dictionaryDaka
    }
    ,{
        path: '/dictionaryDakaZhuangtai',
        name: '打卡状态',
        component: dictionaryDakaZhuangtai
    }
    ,{
        path: '/dictionaryGerenjihua',
        name: '个人计划类型',
        component: dictionaryGerenjihua
    }
    ,{
        path: '/dictionaryGonggao',
        name: '公告类型',
        component: dictionaryGonggao
    }
    ,{
        path: '/dictionaryHuiyi',
        name: '会议类型',
        component: dictionaryHuiyi
    }
    ,{
        path: '/dictionaryQingjia',
        name: '请假类型',
        component: dictionaryQingjia
    }
    ,{
        path: '/dictionaryQingjiaYesno',
        name: '审核状态',
        component: dictionaryQingjiaYesno
    }
    ,{
        path: '/dictionaryQiyejihua',
        name: '企业计划类型',
        component: dictionaryQiyejihua
    }
    ,{
        path: '/dictionarySex',
        name: '性别类型',
        component: dictionarySex
    }
    ,{
        path: '/dictionaryWaichudengji',
        name: '外出类型',
        component: dictionaryWaichudengji
    }
    ,{
        path: '/dictionaryZhiwei',
        name: '职位',
        component: dictionaryZhiwei
    }


    ,{
        path: '/bumenjihua',
        name: '部门计划',
        component: bumenjihua
      }
    ,{
        path: '/daka',
        name: '上下班打卡',
        component: daka
      }
    ,{
        path: '/dictionary',
        name: '字典',
        component: dictionary
      }
    ,{
        path: '/gerenjihua',
        name: '个人计划',
        component: gerenjihua
      }
    ,{
        path: '/gonggao',
        name: '公告',
        component: gonggao
      }
    ,{
        path: '/huiyi',
        name: '会议',
        component: huiyi
      }
    ,{
        path: '/qingjia',
        name: '请假',
        component: qingjia
      }
    ,{
        path: '/qiyejihua',
        name: '企业计划',
        component: qiyejihua
      }
    ,{
        path: '/tongxunlu',
        name: '通讯录',
        component: tongxunlu
      }
    ,{
        path: '/waichudengji',
        name: '外出登记',
        component: waichudengji
      }
    ,{
        path: '/yuangong',
        name: '员工',
        component: yuangong
      }


    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
