    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yuangongId" name="yuangongId" type="hidden">
               ...
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="外出事由" prop="waichudengjiShiyou">
                       <el-input v-model="ruleForm.waichudengjiShiyou"
                                 placeholder="外出事由" clearable  :readonly="ro.waichudengjiShiyou"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="外出事由" prop="waichudengjiShiyou">
                           <el-input v-model="ruleForm.waichudengjiShiyou"
                                     placeholder="外出事由" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="外出类型" prop="waichudengjiTypes">
                        <el-select v-model="ruleForm.waichudengjiTypes" :disabled="ro.waichudengjiTypes" placeholder="请选择外出类型">
                            <el-option
                                v-for="(item,index) in waichudengjiTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="外出类型" prop="waichudengjiValue">
                        <el-input v-model="ruleForm.waichudengjiValue"
                            placeholder="外出类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="外出详情" prop="waichudengjiContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.waichudengjiContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.waichudengjiContent" label="外出详情" prop="waichudengjiContent">
                            <span v-html="ruleForm.waichudengjiContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yuangongId: false,
                    waichuTime: false,
                    waichudengjiShiyou: false,
                    waichudengjiTypes: false,
                    waichudengjiContent: false,
                    insertTime: false,
                },
                ruleForm: {
                    yuangongId: '',
                ...
                rules: {
                   ...
                   waichudengjiTypes: [
                              { required: true, message: '外出类型不能为空', trigger: 'blur' },
                              {  pattern: /^[1-9][0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   waichudengjiContent: [
                              { required: true, message: '外出详情不能为空', trigger: 'blur' },
                          ],
                   insertTime: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=waichudengji_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.waichudengjiTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`waichudengji/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.waichudengjiCrossAddOrUpdateFlag = false;
                                        this.parent.search();
