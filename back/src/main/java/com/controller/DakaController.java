    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(daka !=null){
            ...
            BeanUtils.copyProperties( daka , view );//把实体数据重构到view中
                ...
                //级联表
                YuangongEntity yuangong = yuangongService.selectById(daka.getYuangongId());
                if(yuangong != null){
    ...
    */
    ...
    public R save(@RequestBody DakaEntity daka, HttpServletRequest request){
        ...
        if(false)
            ...
            daka.setYuangongId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<DakaEntity> queryWrapper = new EntityWrapper<DakaEntity>()
            .eq("yuangong_id", daka.getYuangongId())
        ...
        if(dakaEntity==null){
            Date date = new Date();
            if(daka.getDakaTypes() ==1){//上班卡
                int hours = date.getHours();
                if(hours<9){
                    daka.setDakaZhuangtaiTypes(1);
                }else{
                    daka.setDakaZhuangtaiTypes(2);
                }
            }else{//下班卡
                int hours = date.getHours();
                if(hours<18){
            ...
            daka.setDakaTime(date);
            daka.setInsertTime(date);
            daka.setCreateTime(date);
            dakaService.insert(daka);
            return R.ok();
        }else {
            if(daka.getDakaTypes() ==1){
                return R.error(511,"今日已经打过上班卡");
            }else{
                return R.error(511,"今日已经打过下班卡");
            }
        }
    }
...
//                            dakaEntity.setYuangongId(Integer.valueOf(data.get(0)));   //员工 要改的
//                            dakaEntity.setDakaTypes(Integer.valueOf(data.get(0)));   //打卡类型 要改的
//                            dakaEntity.setDakaZhuangtaiTypes(Integer.valueOf(data.get(0)));   //打卡状态 要改的
//                            dakaEntity.setDakaTime(sdf.parse(data.get(0)));          //打卡时间 要改的
